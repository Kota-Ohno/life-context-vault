...
LCV_RELAY_HANDOFF_SECRET=dev-local-handoff-secret \
...
`GET /mcp` opens a Streamable HTTP SSE receive channel for clients that probe or maintain a server-to-client channel. It requires `Accept: text/event-stream` and a valid bearer token. The Relay emits a short `ready` event with `retry: 5000`, a generated `mcp_sse_*` event id, and then closes the connection. If a later request sends that generated id through `Last-Event-ID`, the Relay recognizes the cursor inside its bounded in-memory SSE window, replays only missed Relay metadata events for the same client/session stream, and emits a new ready event with `resumeSupported: true`, `replayPolicy: metadata_only_per_stream_replay`, and `lastEventIdStored: true`. Unknown client-provided cursors are acknowledged as received but are not stored or echoed. Context Pack bodies, MCP response bodies, Raw Sources, tool results, and client-provided `Last-Event-ID` values are never written to the persisted relay state.
...
The handoff body must include `clientId`, `requestId`, `expiresAt`, `mcpResponse`, and `handoffSignature`; the signature is generated by the local Vault with `LCV_RELAY_HANDOFF_SECRET` and covers the client id, request id, expiry, and MCP response body. The Relay rejects unsigned, mismatched, unconfirmed, or expired handoffs.
...
- For CIMD, the Relay fetches the metadata document as JSON, rejects redirects, caps the document at 128 KiB, verifies that `client_id` matches the OAuth `client_id`, requires the requested `redirect_uri` to be listed in `redirect_uris`, and accepts only public-client token auth (`token_endpoint_auth_method: none` when present).
...
The current CIMD path is public-client PKCE support. It does not verify `private_key_jwt` or confidential-client assertions; add that only if provider certification requires it.
...
- Binding outside loopback requires `LCV_RELAY_HANDOFF_SECRET` so short-lived cached Context Packs cannot be spoofed by an admin-shaped request.
...
- Running provider certification against the real hosted endpoint, including any provider-specific long-lived SSE behavior beyond the current metadata-only resume window.
