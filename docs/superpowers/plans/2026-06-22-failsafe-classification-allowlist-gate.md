...
- **Rust gate reads the projection, not JSON.** Pack items are built from `NativeFactSearchResult`, hydrated by SELECT against the **derived `facts` projection table**. The new fields must flow: canonical JSON → projection column → INSERT → SELECT → struct → row mapper → pack-item builder. **Add the columns with `ensure_column` (idempotent; real precedent at `lib.rs:625-630`) — editing the `CREATE TABLE IF NOT EXISTS facts` DDL is a no-op on existing DBs, and `schema_versions` is written-but-never-read (do NOT use it).** Backfill is triggered by the migration write-back (Task 7): `save_vault_json_with_projection` (`lib.rs:1669`) bumps `vault_state.updated_at`, which makes `sync_normalized_tables_if_stale` (`lib.rs:969`, keyed on `projection_state["vault_state_updated_at"]`) rebuild and repopulate from canonical JSON. **Row mapper treats SQL NULL ⇒ `classified=false`** (fail-closed) — never NULL⇒true.
...
**Files:** Modify `src/vault.ts`, `src/vault.test.ts`. Anchors: extract/`detectSensitivity` sites; `approveCandidate` (~525-552, editedText branch); **`updateFactMetadata` (`vault.ts:952`) — there is NO `updateFactText`; this single function edits text/domain/sensitivity**; build options/caller (~1189-1249); build gate `maxSensitivityIncluded`/`confirmationStatus` (~1305-1324); pack-item build (~1509); retrieval paths `restoreFactToPack` (~1344), `refreshEditedContextPack` (~1367), `contextPackPolicyViolation` (~1614-1657); `normalizeState` (~110-160).
...
- [ ] **Step 3 — implement:**
  - **Classify on every text origin:** replace `detectSensitivity(text)` with `classifySensitivity(text)`, mapping `r.tier→detectedSensitivity`, `r.classified→sensitivityClassified`, `r.confidence→sensitivityConfidence`. On **approve-with-edited-text**, re-run `classifySensitivity` on the FINAL text. In **`updateFactMetadata` (`vault.ts:952`)**: if the edit changes the fact text, re-classify the new text; if the caller supplies a manual `sensitivity` override, set `sensitivityClassified=false` (manual override is unverified ⇒ ineligible). (No `updateFactText` exists — both behaviors live in this one function.)
...
- [ ] **Step 1 — failing tests:** (1) build a pack via the native path from a fact whose canonical JSON has `sensitivityClassified:true,sensitivityConfidence:"high"`; assert the built pack item carries both. (2) **Legacy-schema test:** open a DB whose `facts` projection lacks the columns (pre-upgrade), run the open/migration flow, then assert the projected rows now have the columns AND a pre-column legacy fact maps to `sensitivityClassified=false` (NULL ⇒ false, fail-closed).
- [ ] **Step 2 — run, expect FAIL.**
- [ ] **Step 3 — implement:** (a) add the columns idempotently with `ensure_column(connection, "facts", "sensitivity_classified", "INTEGER")` and `ensure_column(connection, "facts", "sensitivity_confidence", "TEXT")` alongside the existing `ensure_column` calls at `lib.rs:625-630` (also list them in the `CREATE TABLE` block for fresh installs). Do NOT touch `schema_versions`. (b) populate them in the `sync_normalized_tables` INSERT (~775) from canonical fact JSON, defaulting missing ⇒ `(0,"low")`; (c) add both fields to `NativeFactSearchResult` (~109) and every fact SELECT (~1093-1145, ~3676, ~3849) + `row_to_native_fact_search_result` (~1056), mapping **SQL NULL ⇒ `classified=false`, `confidence="low"`** (fail-closed); (d) emit them on the pack item (~1256-1270) from the fact (not by re-classifying at build). **The projection backfill on existing vaults is triggered by Task 7's migration write-back** (`save_vault_json_with_projection` bumps `updated_at` → `sync_normalized_tables_if_stale` rebuilds) — adding empty columns alone does NOT backfill; Task 6 alone leaves legacy rows NULL (which read fail-closed, so safe but review-heavy until Task 7 runs).
...
**Files:** Modify `src-tauri/src/lib.rs`: build gate (~1277-1285); auto-delivery release (~1399); retrieval `ensure_context_pack_allowed_by_current_policy` (~3366) / `safe_context_pack_for_client` (~3309); **a NEW `migrate_classification_if_needed(connection)` invoked once in the open flow (`open_vault_db_at_path` ~397) — there is NO existing co-located on-open normalize/migrate site (`load_vault_json_from_connection` ~1597 is a pure read; the standing-delivery test at ~1573-1593 asserts no backfill), so this hook is added, not anchored**; **the real fact-edit command `update_fact_metadata_at_path` (~2895-2934)** (NOT the approve constructor at ~3037) + the approve-with-edited-text site + `context_pack_item_from_fact` (~3581); the write-back `save_vault_json_with_projection` (~1669); bar accessor mirroring `policy_requires_approval_above_for_client` (~6471); `mod tests`.
...
- [ ] **Step 3 — implement:**
  ...
  - **Migration hook (added, not anchored):** add `fn migrate_classification_if_needed(connection)` and invoke it ONCE in the open flow (`open_vault_db_at_path` ~397, after the connection + `vault_state` row exist). It: reads canonical `vault_state`; if the persisted `classifierMigrationVersion` (a NEW field in `vault_state`, NOT the always-rewritten `version`) is below `CURRENT`, defaults missing classification fields + runs the guarded one-shot `reclassify` over legacy facts, sets `classifierMigrationVersion = CURRENT`, and **writes back via `save_vault_json_with_projection` (~1669)** — which bumps `updated_at` and so triggers `sync_normalized_tables_if_stale` to rebuild the projection (this is the real backfill mechanism for Task 6's new columns). If the marker is already current, it is a **no-op** (no write, no `updated_at` churn) so subsequent opens don't re-run. Keep the marker name/semantics identical to the TS `normalizeState` path.
  - **Edit-fact** at the REAL command `update_fact_metadata_at_path` (~2895-2934): if the edit changes `factText`, re-run `classify_sensitivity` on the new text and set the fields from it; if the caller supplies a manual `sensitivity`, set `sensitivityClassified=false` (unverified override). Do NOT wire this at ~3037 (that block is the approve-with-edited-text constructor — handle approve reclassify there separately). Note the asymmetric naming: TS `updateFactMetadata` ↔ Rust `update_fact_metadata_at_path`.
- [ ] **Step 4 — run, expect PASS** — `cargo test --manifest-path src-tauri/Cargo.toml`.
...
- **Fresh-eyes findings → tasks (round 1):** projection path → T6; always_review short-circuit → T4,7 (verbatim in constraints); retrieval enforcement → T4,7; per-item fold → constraints + T4,7 + mixed-pack tests; bar threading → T4,7; Rust migration write-back/one-shot → T7; edit-fact fail-open → T4,7; per-signal confidence → T1,5 constraints; high-tier recall/priority → T1,5 parity tests; one-shot marker (not `version`) → T4,7 (`classifierMigrationVersion`); existing-test breakage → T8; non-compiling intermediate → T3 optional/T4 tighten; empty-pack vacuity → constraints + T4 test.
- **Fresh-eyes findings → tasks (round 2, verified against real code):** the "bump schema version → rebuild" mechanism is fictional → T6 now uses `ensure_column` (precedent `lib.rs:625-630`) + backfill via Task 7's `save_vault_json_with_projection` write-back; SQL NULL ⇒ classified=false (fail-closed) pinned in constraints + T6 legacy-DB test. Edit-fact mis-anchor → T4/T7 re-anchored to `update_fact_metadata_at_path` (~2895) / `updateFactMetadata` (`vault.ts:952`); `updateFactText` does not exist (removed). Nonexistent shared migration site → T7 adds `migrate_classification_if_needed` invoked in `open_vault_db_at_path` (~397), writing back via `save_vault_json_with_projection` (~1669), marker-guarded for idempotent no-op on later opens.
